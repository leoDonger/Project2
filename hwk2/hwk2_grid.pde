...
void setup() {
  ...
  camera = new Camera();
  camera.position = new PVector(172, 296, -2563);
  camera.theta = -121.53;
  camera.phi = 0.266;
  smooth(8);
  ...
  for (int i = 0; i < 5; i++) {
    for (int j = 0; j < 5; j++) {
      nodes_mesh[i][j] = new Node(new Vec2(base_pos.x + j * link_length, base_pos.y));
    }
  }
}
...
void draw() {
  ...
  for (int i = 0; i < nodes_mesh.length; i++) {
    ...
    if (i < nodes_mesh.length-1){
      pushMatrix();
      line(nodes_mesh[i][0].pos.x * scene_scale, nodes_mesh[i][0].pos.y * scene_scale, (-50 - i * link_length) * scene_scale, nodes_mesh[i+1][0].pos.x * scene_scale, nodes_mesh[i+1][0].pos.y * scene_scale, (-50 - (i+1) * link_length) * scene_scale);
      popMatrix();
    }
  }
